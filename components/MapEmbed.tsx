...
export default function MapEmbed({ className, contentClassName, variant = 'card' }: MapEmbedProps = {}) {
  ...
  const wrapperClassName = cn(
    variant === 'card' ? 'card overflow-hidden' : 'overflow-hidden rounded-[1.75rem] border border-border/70',
    className,
  );

  return (
    <div className={wrapperClassName}>
      <MapEmbedClient className={contentClassName} mapSrc={siteSettings.mapsUrl} previewImage={MAP_PREVIEW_IMAGE} />
    </div>
  );
}
