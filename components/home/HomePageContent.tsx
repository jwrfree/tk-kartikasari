...
export default function HomePageContent(props: HomePageContentProps) {
  const { schoolName, stats, credentials, timeline, blogPosts, testimonials } = props;
  ...
  return (
    <main>
      <EditorialHero
        ...
        secondaryAction={
          <Button asChild variant="outline">
            <Link href="#kegiatan-harian">{homeHero.secondaryCtaLabel}</Link>
          </Button>
        }
        facts={heroFacts}
        mediaCaption="Sebelum mendaftar, keluarga sebaiknya bisa melihat dulu suasana sekolah, ritme pagi, dan orang-orang yang akan ditemui anak setiap hari."
      />
      ...
      <StorySection
        ...
        aside={
          <QuoteBlock
            quote="Kepercayaan orang tua biasanya tumbuh bukan saat mendengar janji besar, tetapi saat proses kecil sehari-hari terasa jelas."
          ...
          {redesignedPrograms.map((program) => (
            <CardSurface key={program.name} tone="translucent" padding="xl" className="space-y-5">
              <div className="flex flex-col gap-3 border-b border-border/60 pb-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-secondary">{program.age}</p>
          ...
          {homeOnboardingCopy.steps.map((step, index) => (
            ...
            >
              ...
              <div>
                ...
                <Link href={step.href} className="mt-4 inline-flex text-sm font-semibold text-primary transition hover:text-primary/80">
                  {step.linkLabel}
                </Link>
              </div>
            </div>
      ...
      <StorySection
        eyebrow={homeBlogCopy.eyebrow}
        title={homeBlogCopy.title}
        description={homeBlogCopy.description}
      >
        {blogPosts.length > 0 ? (
          <div className="grid gap-5 md:grid-cols-3">
            {blogPosts.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
                <CardSurface tone="translucent" padding="lg" className="flex h-full flex-col justify-between gap-5 transition group-hover:-translate-y-1">
                  <div className="space-y-3">
